...
- **Board & To-Do views**: give a note a status (Backlog / In Progress / Done) and it becomes a card on the **Kanban board** (drag between columns) and a checkable item in the **To-Do** list — separate views from the markdown editor.
- **Per-folder Git sync (GitLab)**: link any folder to a Git repo and **manually** Pull / Commit&Push it (no auto-push), with branch selection. Auth is a GitLab access token over HTTPS (stored `0600`, sent via a request header — never in the URL/reflog). Point a folder at a normal repo or a `…/project.wiki.git` so a team can read/edit it in GitLab's Wiki UI. Needs `git` on PATH; the rest of the app works without it.
