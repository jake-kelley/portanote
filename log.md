...
- **2026-08-17 — v1.6.12**: select several lines and press Tab to indent them all, Shift+Tab to unindent — handy for nesting a code block under a numbered list item. Tab previously replaced the whole selection with two spaces, deleting the highlighted lines. New `⇥` and `⇤` toolbar buttons do the same thing. Also on macOS: Eisvogel PDF export no longer fails with an unexplained `Operation not permitted` panic — tectonic's package cache moves to `~/Library/Caches/Portanote/tectonic`, out of the protected `~/Documents`, and a permissions failure now says so instead of printing a raw Rust backtrace.
- **2026-08-16 — v1.6.11**: share a note with another Portanote user, and import notes from anywhere. Share gives you a pasteable code for a small note, or a `.portanote` file when it carries images — and on Windows that file goes straight onto the clipboard, so Ctrl+V attaches it in Teams or Slack. Import takes bundles, codes, plain Markdown from Obsidian or Hugo, or a whole dropped folder with its images, into the folder you pick.
